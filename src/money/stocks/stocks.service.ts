...
export class StocksService {
  ...
  async findAll(userId: string) {
    return this.prisma.stockHolding.findMany({
      where: { userId },
  ...
  async create(userId: string, dto: CreateStockDto) {
    return this.prisma.stockHolding.create({ data: { ...dto, userId } });
  }

  async update(userId: string, id: string, dto: UpdateStockDto) {
    await this.verifyOwner(userId, id);
    return this.prisma.stockHolding.update({ where: { id }, data: dto });
  }
