...
## CI/CD 파이프라인 — 배포 자동화 (다음 단계, 인프라 구성 예정)
...
3. 단위/E2E 테스트
4. 빌드·배포 전 테스트
5. GitHub Actions에서 빌드·테스트 후 배포

### 체계적인 곳은 보통 이렇게 함

1. **코드 작성** (로컬)
2. **로컬 단위 테스트** — 커밋 전에 `npm run test` (watch 모드로 저장할 때마다 자동 실행하는 경우도 많음)
3. **브랜치 → PR 생성** — 혼자 하는 프로젝트여도 `main`에 바로 push 안 하고 브랜치/PR을 거치는 경우가 많음 (기록 + CI 트리거 용도)
4. **CI가 자동 실행** (사람이 수동으로 다시 안 돌림) — PR 올리는 순간 GitHub Actions가 lint → build → 단위 테스트 → E2E까지 실행. 하나라도 실패하면 **머지 자체가 막힘** (브랜치 보호 규칙)
5. **머지 → 배포** — `main` 머지 시 배포 워크플로우 실행 (또는 승인 스텝 하나 더)
6. **배포 후 스모크 테스트** — 배포 직후 헬스체크/핵심 API 하나만 빠르게 호출해서 "배포가 살아있는지" 확인

### 핵심 차이

> 사람이 배포 직전에 수동으로 테스트를 "한 번 더" 도는 게 아니라, **CI가 테스트를 실행하고 통과해야만 배포 단계로 넘어가도록 파이프라인 자체를 구성**한다.

이렇게 하면 "테스트 깜빡하고 배포"하는 실수가 구조적으로 불가능해짐. 즉 4번(배포 전 테스트)과 5번(GitHub Actions 빌드·테스트·배포)을 **하나의 자동화된 파이프라인**으로 합치는 게 핵심.

### 이 프로젝트(1인 사이드 프로젝트)에 맞는 현실적인 버전

- PR 단계는 생략 가능(오버킬) → `main` push 시 GitHub Actions가 바로 실행
- 파이프라인 단계: **lint → build → 단위 테스트 → E2E 테스트(Neon dev 브랜치 DB 사용) → (전부 통과 시) Render 배포**
- 각 단계는 이전 단계가 성공해야만 실행되도록 구성 (`needs:` 또는 `if: success()`)
- 배포는 Render의 GitHub 연동 자동배포를 그대로 쓰되, **CI가 실패하면 애초에 그 커밋이 배포 브랜치까지 못 가게** 구성하는 게 이상적 (현재는 push하면 바로 Render가 배포를 트리거하므로, 추후 "CI 통과 후에만 배포되는 브랜치 전략"으로 보완 필요)

### 인프라 구성 체크리스트 (착수 전 To-do)

- [ ] `.github/workflows/ci.yml` 작성 — lint/build/test job
- [ ] CI용 테스트 DB 연결 — Neon dev 브랜치 URL을 GitHub Actions Secret으로 등록
- [ ] E2E 테스트가 실제로 존재해야 CI에서 의미가 있음 → 금융 지식 CRUD E2E부터 작성
- [ ] 브랜치 전략 결정 — `main` 바로 push 유지할지, `dev` 브랜치 두고 CI 통과 후 `main` 머지할지
- [ ] Render 배포 트리거 방식 재검토 — 지금은 push 즉시 배포, CI 게이트를 실제로 걸지 결정
