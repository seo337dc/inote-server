...
### Node.js 탄생 배경 — 왜 만들어졌나

전통적인 서버(Apache 등)는 **요청 하나당 스레드 하나**를 할당하는 방식이었음.
스레드는 메모리를 많이 먹어서(개당 1MB+), 접속자가 수천 명이면 스레드도 수천 개 →
메모리 부족·전환 비용 폭증 (이걸 "C10K 문제"라고 부름).

Node.js는 **"요청마다 스레드를 새로 만들지 말고, 스레드 하나로 수천 개를 돌려막자"**는
아이디어로 접근함.

### 싱글 스레드 + 이벤트 루프 — 핵심 아키텍처

**JS 코드 실행 자체는 싱글 스레드.** 두 JS 코드가 동시에 도는 일은 없음.
그런데 수천 개 요청을 어떻게 동시에 처리하나?

핵심은 **"기다리는 작업"(I/O)을 스레드가 직접 붙잡고 기다리지 않는다는 것**:

1. DB 쿼리를 날리면 "끝나면 이 콜백 불러줘" 하고 바로 다음 코드로 넘어감 (기다리지 않음)
2. 실제 쿼리는 `libuv`(내부 C 라이브러리)가 백그라운드에서 처리
3. 끝나면 결과가 콜백 큐에 쌓임
4. **이벤트 루프**가 "지금 실행 중인 코드가 없으면 큐에서 하나 꺼내 실행"을 무한 반복
...
동기 코드(`1`, `3`)가 무조건 먼저 다 실행되고, 비동기 콜백(`2`)은 그 뒤에야 실행됨.
"0초 뒤"는 "지금 당장"이 아니라 "동기 코드 다 끝나면 최대한 빨리".

### 왜 서버에 유리한가 / 약점

웹서버 작업은 대부분 **CPU 계산보다 "뭔가 끝나길 기다리는 시간"(I/O 대기)**이 훨씬 김
(DB 응답, 다른 API 응답, 파일 읽기 등). I/O 대기는 스레드가 안 붙잡고 있어도 되니까,
**스레드 하나로 수천 개의 "대기 중인 요청"을 동시에 관리** 가능.

반대로 **CPU를 오래 붙잡는 계산**(암호화, 대용량 이미지 처리)을 하면 그동안 이벤트 루프
전체가 멈춤 — 이런 무거운 계산엔 안 맞고, `Worker Threads`로 우회함.

### 이벤트 루프의 단계(phase)

이벤트 루프는 사실 "콜백 큐 하나"가 아니라 **여러 단계(phase)를 정해진 순서로 도는 구조**:

| 단계 | 여기서 실행되는 것 |
|---|---|
| **timers** | `setTimeout`, `setInterval` 콜백 |
| **poll** | I/O 콜백(파일 읽기 완료, 네트워크 응답 등) — 대부분의 콜백이 여기서 실행 |
| **check** | `setImmediate` 콜백 |

"timers 다 처리 → poll 다 처리 → check 다 처리 → 다시 처음으로"를 반복.

### 매크로태스크 vs 마이크로태스크

- **매크로태스크**: `setTimeout`, `setInterval`, `setImmediate`, I/O 콜백
- **마이크로태스크**: `Promise.then/catch/finally`, `queueMicrotask`

**규칙: 매크로태스크 하나 실행 후, 다음 매크로태스크로 넘어가기 전에 마이크로태스크
큐를 완전히 비운다.** 마이크로태스크가 항상 매크로태스크보다 먼저 끼어듦.

Node.js에만 있는 `process.nextTick`은 마이크로태스크보다도 더 먼저 실행됨.

**우선순위: `process.nextTick` > `Promise.then`(마이크로태스크) > `setTimeout`/`setImmediate`(매크로태스크)**
...
1. **A, E** — 동기 코드는 절대 안 끊기고 위에서 아래로 먼저 다 실행됨
2. **D** — 동기 코드 끝나자마자 `process.nextTick` 큐부터 확인
3. **C** — `nextTick` 큐 비운 뒤 마이크로태스크 큐(`Promise.then`) 확인
4. **B** — 마이크로태스크까지 다 비운 뒤에야 다음 매크로태스크(`setTimeout`)로 넘어감

> **다음 학습 (이어서 할 것):** Express — 라우트(`app.get(...)`)와 미들웨어
> (`app.use(...)`, `next()`) 개념. NestJS의 `@Get()`/`@UseGuards()`가 사실 이 위에
> 얹힌 것이라는 연결까지.
...
## Chapter 25 — 다음 학습 계획: 인증(Auth) 기초 다지기

> ⚠️ 학습 진행 중, 2026-09-09부터 잠시 보류 — 1번(인증 흐름)+CORS 심화는 완료, 2번(세션
> 기반 인증)은 중간까지. `inote-blog`에 회원가입/로그인/회원탈퇴를 실제로 붙이기 전에 FE/BE
> 인증 프로세스를 상세하게 이해하고 가기로 했던 계획 — 남은 항목은 아래 "재개 체크리스트" 참고,
> 지금은 기능 개발 우선.

### 배경

- 방향은 정함: **Google + 이메일/비밀번호 둘 다 지원**
- "JWT로 적용해야 할 것 같다"는 감이 있는데, 이게 실제로 맞는 판단인지 아직 확신 없음
  → **학습을 통해 스스로 판단하는 게 목표.** 지금은 결론 안 내고 넘어감
- 스스로 진단한 약점: "FE/BE/인증 프로세스가 알고는 있지만 상세하게는 모른다"
  → 그래서 Better Auth API 사용법보다 **한 단계 아래, 인증의 기본 원리**부터 다지기로 함

### 학습 순서 (하나씩, 확인하면서)

1. **인증(Authentication) 전체 흐름 — 브라우저 관점**
   - 로그인 버튼 클릭 → 서버 응답 → 브라우저에 뭐가 저장되나 → 다음 요청부터
     "이게 나야"를 어떻게 증명하나
   - 쿠키(Cookie)가 뭐고, 왜 요청마다 자동으로 실려가는지

2. **세션(Session) 기반 인증 — 지금 Better Auth가 쓰는 방식**
   - Chapter 15(이미 있음) 복습 + 실제로 DB `session` 테이블에 뭐가 저장되는지 직접 확인
   - 로그인 → 세션 생성 → 쿠키엔 세션 ID만 → 요청마다 DB 조회해서 검증, 이 흐름을
     코드 레벨(`auth.guard.ts`)로 다시 짚기

3. **JWT 기반 인증 — 세션과 뭐가 다른가**
   - 서명(sign)이 뭔지, 왜 DB 조회 없이 검증 가능한지
   - "JWT는 취소가 어렵다"가 실제로 무슨 뜻인지 (탈취당하면 만료 전까진 막을 방법이
     마땅치 않다는 것)

4. **"지금 우리 상황에 JWT가 진짜 필요한가?" 직접 판단해보기**
   - 지금 붙이려는 게 웹 브라우저 로그인인지, 다른 이유가 있는 건지 정리
   - 이미 RN 앱용 `oneTimeToken` 플러그인(쿠키 교환용, Chapter 12 이전 참고)이 있는데,
     이거랑 "JWT 도입"이 같은 이야기인지 다른 이야기인지 구분해보기

5. **이메일/비밀번호 인증 추가하는 법**
   - Better Auth의 `emailAndPassword` 옵션 (지금은 `socialProviders.google`만 있음)
   - 비밀번호는 실제로 어떻게 저장되나 (평문 저장 금지, 해싱 — bcrypt 등)

6. **회원탈퇴가 실제로 뭘 지우는지**
   - 유저 레코드만 지우면 되나, 연관된 글/설정도 같이 지워야 하나
   - Prisma `onDelete: Cascade`가 이미 각 모델에 걸려있는데 이게 정확히 뭘 보장해주는지

7. **(별도 스텝으로 분리) XSS(Cross-Site Scripting)** — 2026-09-08, 1번(인증 흐름) 학습 중
   `HttpOnly` 얘기가 나오면서 XSS가 언급됐는데, 이건 인증 흐름 자체보다 큰 주제라 별도 스텝으로
   빼서 나중에 상세히 학습하기로 함. (예: 어떤 입력 지점이 취약한지, sanitize/escape가 실제로
   뭘 하는지, CSP 헤더, React가 기본적으로 막아주는 부분과 안 막아주는 부분 구분 등)

### 오늘 결정하고 넘어간 것 (참고)

- 로그인 방식: Google + 이메일/비밀번호 둘 다
- BE `trustedOrigins`에 `inote-blog` 추가 필요 (아직 안 함 — 위 학습 끝나고 실제 구현 시작할 때 첫 스텝)

### 학습 진행 기록

**2026-09-08**

- **1번(인증 전체 흐름) 학습 완료** — HTTP 무상태성 → 로그인 시 세션 생성+`Set-Cookie` → 브라우저의
  자동 쿠키 첨부 → 크로스 오리진일 땐 `credentials:'include'`+CORS 필요 → 쿠키 속성
  (`HttpOnly`/`Secure`/`SameSite`) → 서버의 세션 검증까지 확인. 퀴즈(HttpOnly 없으면 세션을 어떻게
  훔칠 수 있나 → `document.cookie`+XSS)도 직접 맞춤.
- **CORS 상세 학습 완료** (1번에서 파생된 심화 주제) — Same-Origin Policy가 뭘 막는지(응답 읽기,
  요청 자체는 못 막음) → 출처(origin) = 스킴+호스트+포트 정의 → `evil.com` 시나리오(브라우저가
  쿠키를 자동으로 실어 보내기 때문에 위험함, origin을 몰라서가 아니라) → `Access-Control-Allow-Origin`
  헤더로 서버가 허용 → `credentials:true`일 땐 `origin:'*'` 금지(스펙 강제)인 이유 → Preflight
  (`OPTIONS`)가 도는 조건("단순 요청" 3조건: 메서드 GET/HEAD/POST, 단순 Content-Type,
  커스텀 헤더 없음 — 다 만족해야 preflight 생략) → `SameSite`(쿠키가 실리는지)와 CORS(응답을
  읽을 수 있는지)가 **서로 다른 레이어**라는 것, 로컬 http/프로덕션 https 크로스 오리진에서 왜
  다르게 분기해야 했는지 → Better Auth `trustedOrigins`는 CORS가 아니라 OAuth 콜백 검증용 별도
  화이트리스트라는 것까지 확인. 전부 퀴즈로 직접 확인하며 진행, 다 맞춤.
- **2번(세션 기반 인증) 진행 중, 중단 지점 있음** — 실제 `inote-server` 코드로 확인: DB `session`
  테이블 구조(`token`이 쿠키에 들어가는 값, `userId`로 유저 연결, `ipAddress`/`userAgent`는
  메타데이터) + `auth.guard.ts`를 한 줄씩 읽음 (`fromNodeHeaders` → `auth.api.getSession` →
  DB에서 `token` 매칭 → 없으면 401, 있으면 `request.user`에 부착). **다음에 이어서 풀 질문**:
  "로그아웃하면 서버가 정확히 뭘 해야 이 유저가 더 이상 로그인 상태가 아니게 되나? (쿠키는
  브라우저에 남아있다고 가정)" — 아직 답 안 함.

**2026-09-09 — 학습 잠시 보류, 기능 개발 우선으로 전환**

- 2번 나머지(로그아웃 메커니즘) + 3~7번(JWT, JWT 필요성 판단, 이메일/비밀번호 인증, 회원탈퇴
  cascade, XSS)은 아직 학습 전. 지금은 학습보다 기능 개발에 집중하기로 함 — 아래를
  **재개용 체크리스트**로 남겨둠.

### 다음에 이어서 학습할 것 (재개 체크리스트)

- [ ] 2번 마무리 — 로그아웃 시 서버가 뭘 해야 하는지 (DB `session` row 삭제 vs 그냥 만료 대기)
- [ ] 3번 — JWT 기반 인증 (서명, DB 조회 없이 검증되는 원리, 취소가 어렵다는 것의 실제 의미)
- [ ] 4번 — 지금 상황에 JWT가 진짜 필요한가 직접 판단 (`oneTimeToken`과 "JWT 도입"이 다른 얘기인지 구분)
- [ ] 5번 — Better Auth `emailAndPassword` 옵션, 비밀번호 해싱
- [ ] 6번 — 회원탈퇴 시 `onDelete: Cascade`가 뭘 보장하는지
- [ ] 7번 — XSS (입력 지점, sanitize/escape, CSP, React가 기본으로 막아주는 부분/안 막아주는 부분)
